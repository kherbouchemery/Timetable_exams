<?php
    ...
    $aColumns = array( 'id', 'last_name', 'first_name', 'degre', 'email', 'dep', 'type', 'tel');
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            else if ( $aColumns[$i] != ' ' )
            {
                // dep (id)->(nom)
                if($aColumns[$i] == 'dep'){
                    $query = $link->query("SELECT * FROM departement WHERE id = ".$aRow['dep']);
                ...
                if($i == count($aColumns)-1){
                    ...
                    $query = $link->query("SELECT * FROM tablesheet WHERE teacher_id = ".$aRow['id']);
                    $query->setFetchMode(PDO::FETCH_ASSOC);
                    
                    $vh = 0;
                    while($result = $query->fetch()){
                        switch($result['type_seance']){
                            case 'TD':
                                $vh++;
                                break;
                            case 'TP':
                    ...
                    $query = $link->query("SELECT * FROM cours_communs WHERE teacher_id = ".$aRow['id']);
                    $query->setFetchMode(PDO::FETCH_ASSOC);
                    while($result = $query->fetch())
                        $vh += 1.5;
                    $row[] = $vh;
