<?php
    ...
    $aColumns = array( 'course_id', 'course_name', 'course_full_name', 'dep_id');
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            {
                if($aColumns[$i] == 'dep_id'){
                    $query = $link->query("SELECT * FROM departement,faculty WHERE id = ".$aRow['dep_id']." AND faculty.faculty_id = departement.faculty_id");
                    $query->setFetchMode(PDO::FETCH_ASSOC);
                   
                    if($result = $query->fetch()){
                        $row[] = $result['faculty_id'];
