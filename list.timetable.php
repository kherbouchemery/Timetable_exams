<?php
    ...
    $aColumns = array( 'timetable_id', 'course_id', 'groupe_id', 'year', 'semester', 'timing_course_id');
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            {
                if($aColumns[$i] == 'course_id' || $aColumns[$i] == 'timing_course_id'){
                    $row[] = $aRow[ $aColumns[$i] ];

                    $query = $link->query("SELECT * FROM course WHERE course_id = ".$aRow[$aColumns[$i]]);
                ...
                if($aColumns[$i] == 'groupe_id'){
                    $query = $link->query("SELECT * FROM groupe WHERE id = ".$aRow['groupe_id']);
                    $query->setFetchMode(PDO::FETCH_ASSOC);
