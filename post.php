<?php
   ...
   function count_sub($celldata,$timetable_id, $user_id){
	   ...
	   $db_connection = new dbConnection();
	   $link = $db_connection->connect();
	   $query = $link->query("SELECT * FROM tablesheet WHERE data='$celldata' AND
	...
	function check_faculty_collision($faculty_name, $cellid, $user_id){
		$db_connection = new dbConnection();
	    $link = $db_connection->connect();
	    $query = $link->query("SELECT * FROM tablesheet WHERE cell='$cellid'
        AND faculty_name='$faculty_name' AND user_id='$user_id'");
			...
			if(strcmp(@$r, $celldata) == 0)
			{
				...
				if(count_sub($celldata,$timetable_id,$user_id)<count_ltp($celldata,$user_id)){
					...
					if(check_faculty_collision($faculty_name, $cellid, $user_id)){
						...
						$query = $link->prepare("INSERT INTO tablesheet(`cell`, `data`,`prof`, `salle`,`faculty_name`, `timetable_id`, `user_id`) VALUES(?,?,?,?,?)");
